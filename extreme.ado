...
program extreme, eclass byable(recall) sortpreserve
	...
	if inlist(`"`1'"', "gpd", "gev") {
		...
		fvrevar `varlist' if `touse', tsonly
		...
		foreach param in mu sig xi {
			if "`param'"!="mu" | `GEV' {
				...
				fvrevar `r(varlist)' if `touse', tsonly
				local `param'_revar `r(varlist)'
			}
		}
		...
		foreach thresh `=cond(0`Nthresh'>1, "of numlist", "in")' `_threshold' {
			...
			ml model lf2 extreme_lf2() `=cond(`GEV', "(mu:`mu_revar',`munocons')", "")' (lnsig:`sig_revar',`signocons') (xi:`xi_revar',`xinocons') `wgt', ///
				collinear title(`title') `vce' constraints(`constraints') nopreserve // estimate directly even when bootstrapping, in order to rescue e(ll)
